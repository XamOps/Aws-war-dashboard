...
def check_public_s3_buckets():
    """
    Identifies S3 buckets that are publicly accessible.
    Pillar: Security
    Required IAM Permissions: s3:GetBucketPolicyStatus, s3:GetBucketAcl
    """
    s3 = boto3.client('s3')
    public_buckets = []
    try:
        ...
        for bucket in buckets:
            ...
            try:
                ...
                # This error means no bucket policy exists, which is not public.
...
def check_unrestricted_security_groups():
    """
    Finds security group rules that allow unrestricted access from 0.0.0.0/0.
    Pillar: Security
                ...
                for ip_range in rule.get('IpRanges', []):
                    if ip_range.get('CidrIp') == '0.0.0.0/0':
                        risky_rules.append({
                            ...
                            "PortRange": rule.get('FromPort', 'All'),
...
def check_cloudtrail_status():
    """
    Verifies that at least one multi-region CloudTrail trail is active.
    Pillar: Security
    Required IAM Permissions: cloudtrail:DescribeTrails
    """
...
def check_ebs_backup_status():
    """
    Checks all 'in-use' EBS volumes for recent snapshots (within 7 days).
    Pillar: Reliability
    Required IAM Permissions: ec2:DescribeVolumes, ec2:DescribeSnapshots
    """
    ec2 = boto3.client('ec2')
...
def check_ec2_detailed_monitoring():
    """
    Identifies EC2 instances that do not have Detailed Monitoring enabled.
